...
2. zipファイルを解凍し、Setup.exeを起動。指示に従ってインストールを実行（アンインストールする場合は、Windows設定の「インストールされているアプリ」からDataViewerSetupを選択し、アンインストールする 
3. zipファイルを展開し、DataViewer.exeを起動（初回起動時は作業フォルダ作成の確認メッセージが表示される）
4. データファイルを右下のリストにドラッグ＆ドロップ
...
8. 結果の確認
	...
	- ファイルリストの項目を選択し、\[Delete\]をクリックすると選択した項目を削除する 
9. プロット詳細設定
	...
	- \[Apply Settings to Others\]をクリックすると、選択されたデータ項目のプロット設定が他のすべての項目に反映される
	- \[Delete all\]をクリックすると、リストのデータ項目をすべて削除する
	- ファイルリストの項目を選択し、\[Phase Animation\]をクリックすると、複素数の偏角を変えながらアニメーションを表示する（複素数データのみ指定可能）
